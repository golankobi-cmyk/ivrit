  ...
  e.waitUntil(
    caches.keys().then(function (keys) {
      return Promise.all(keys.filter(function (k) {
        return k.indexOf('ivrit-shell-') === 0 && k !== CACHE;
...
self.addEventListener('fetch', function (e) {
  ...
  if (url.origin !== self.location.origin) return;  // bypass cross-origin (Supabase, Anthropic, fonts)
  // Network-first: always prefer fresh content when online so updates load; fall back to cache offline.
  e.respondWith(
